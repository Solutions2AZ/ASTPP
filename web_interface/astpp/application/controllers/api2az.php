...
// ##############################################################################
class Api2az extends MX_Controller {
	function Api2az() {
		parent::__construct ();
	...
	function getnextfreeaccount($reference = '', $resellerId = '', $prefixNumber = '') {
		//echo "Reference: " . $reference . " ResellerId: " . $resellerId . " Prefix Number: " . $prefixNumber;
		$where = array (
				'reseller_id' => $resellerId,
				'status' => '1',
				'reference' => ''
		);
		$accountid_arr = $this->db_model->getSelect ( 'id,number', 'accounts', $where );

		if ($accountid_arr->num_rows () == 0) {
			echo "No free numbers"; //TODO Generate notification o create news
		}
		else if ($accountid_arr->num_rows () > 0) {
			$accountid_arr = $accountid_arr->result_array ();
			$accountid = $accountid_arr [0] ['number'];
			$this->db->update ( "accounts", array (
									"reference" => $reference,
									"status" => '0' 									
							), array (
									"id" => $accountid_arr [0] ['id'] 
							) );
			echo $accountid;
		} else {
			echo "ERROR";
	...
	function getcalls($accountNumber = '', $resellerId = '') {
		$where = array (
				'reseller_id' => $resellerId,
		...
		if ($accountid_arr->num_rows () == 0) {
			...
			$infoCalls = $this->db_model->getSelect ('*', 'cdrs', $where ); //uniqueid,callednum,billseconds,callstart,call_direction,calltype,country_id,debit,cost
			$infoCalls = $infoCalls->result_array ();
			header('Content-Type: application/json; charset=utf-8');
			echo json_encode($infoCalls);
			//echo $accountid;
		} else {
			echo "ERROR";
		}
	...
	function getcountries() {
		$where = array ();
		$infoCountries = $this->db_model->getSelect ('*', 'countrycode', $where );
		$infoCountries = $infoCountries->result_array ();
		header('Content-Type: application/json; charset=utf-8');
		echo json_encode($infoCountries);
	}
